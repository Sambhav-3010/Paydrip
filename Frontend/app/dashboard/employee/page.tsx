...
export default function EmployeeDashboard() {
  ...
  const totalAvailableEth = streams.reduce((sum, stream) => sum + stream.withdrawableEth, 0)
            ...
            <section className="brutal-card overflow-hidden bg-card/95">
              <div className="grid gap-6 border-b-[3px] border-border px-6 py-6 lg:grid-cols-[minmax(0,1fr)_260px] lg:px-8 lg:py-8">
                <PageHeader
                  ...
                  description="Track live accrual, monitor available balance, and withdraw salary streams with confidence."
                />
                ...
                <div className="grid grid-cols-2 gap-3 self-start lg:grid-cols-1">
                  ...
                  <div className="brutal-inset px-4 py-3">
                    <p className="brutal-kicker">Ready to Withdraw</p>
                    <p className="mt-2 brutal-mono text-2xl font-bold text-foreground">{totalAvailableEth.toFixed(3)} ETH</p>
                  </div>
                </div>
              </div>
                ...
                {!isConnected && (
                  <div className="brutal-card bg-secondary p-8 text-center space-y-4">
                    ...
                    <p className="mx-auto max-w-2xl text-sm text-muted-foreground">
                      Connect your wallet to load salary streams, view accrued funds in real time, and unlock withdraw actions.
                    </p>
                  ...
                  <>
                    ...
                    {isLoading ? (
                      <div className="brutal-card bg-secondary p-10 text-center">
                        ...
                        <p className="mt-3 text-lg font-semibold">Fetching employee streams...</p>
                      </div>
                    ) : (
                      <>
                          ...
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                              ...
                              <div className="mt-2 flex items-center gap-4">
                                <h2 className="text-2xl font-bold brutal-title">Your Streams</h2>
                                <span className="brutal-tag brutal-tag-primary">{activeStreams.length}</span>
                              </div>
                            </div>
                            <p className="max-w-xl text-sm text-muted-foreground">
                              Review incoming payments, track current withdrawable value, and open stream details in one click.
                            </p>
                          </div>
                        ...
                        {nonActiveStreams.length > 0 && (
                          <div className="space-y-4 pt-4">
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                              ...
                              <p className="max-w-xl text-sm text-muted-foreground">
                                Keep historical streams separate from active payroll so the day-to-day view stays focused.
                              </p>
                            </div>
